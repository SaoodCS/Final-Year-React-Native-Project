...
// COMPONENT: DISPLAYS BOTH THE DEFAULT CARDIO EXERCISES AND USER-CREATED CARDIO EXERCISES IN ONE FLATLIST FOR THE USER TO PRESS ON AND VIEW THEIR PROGRESS ON THE DISPLAYEXPROGRESS COMPONENT

// DB TO GET AND DISPLAY USER CREATED EXERCISES
var db = openDatabase({name: 'userExercises.db'});

const CardioProgress = ({route}, props) => {
  ...
  useEffect(() => {
    db.transaction(tx => {
      tx.executeSql(
        'SELECT * FROM user_exercises WHERE exercise_musclegroup = "Full Body"',
        [],
        (tx, results) => {
          var temp = [];
  ...
  function filterOutRows(database) {
    ...
    const result7 = result6.filter(item => item.MuscleGroup != 'Shoulders');

    return result7;
  }

  const cardioExercises = filterOutRows(exercisesdb);
  ...
  const concatDefaultNSavedExercises = cardioExercises.concat(changeLabels);

  return (
    <View style={{backgroundColor: '#171717', height: 700}}>
      <ScrollView>
        <CustomHeaderWithBack
          pageName="Cardio Exercises Progress"
                  ...
                  <Button
                    ...
                    onPress={() => {
                      navigation.navigate('Display Progress', {
                        nameOfPreviousScrn: 'Cardio Progress',
