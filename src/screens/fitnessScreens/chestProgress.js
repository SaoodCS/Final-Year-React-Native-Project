...
// COMPONENT: DISPLAYS BOTH THE DEFAULT CHEST EXERCISES AND USER-CREATED CHEST EXERCISES IN ONE FLATLIST FOR THE USER TO PRESS ON AND VIEW THEIR PROGRESS ON THE DISPLAYEXPROGRESS COMPONENT

// DB TO GET AND DISPLAY USER CREATED EXERCISES
var db = openDatabase({name: 'userExercises.db'});

const ChestProgress = ({route}, props) => {
  ...
  useEffect(() => {
    db.transaction(tx => {
      tx.executeSql(
        'SELECT * FROM user_exercises WHERE exercise_musclegroup = "Chest"',
        [],
        (tx, results) => {
          var temp = [];
  ...
  const chestExercises = filterOutRows(exercisesdb);
  ...
  const concatDefaultNSavedExercises = chestExercises.concat(changeLabels);

  return (
    <View style={{backgroundColor: '#171717', height: 700}}>
      <ScrollView>
        <CustomHeaderWithBack
          pageName="Chest Exercises Progress"
                  ...
                  <Button
                    ...
                    onPress={() => {
                      navigation.navigate('Display Progress', {
                        nameOfPreviousScrn: 'Chest Progress',
