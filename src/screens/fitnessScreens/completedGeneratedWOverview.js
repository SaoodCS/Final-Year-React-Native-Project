...
// COMPONENT: FOLLOWS FROM THE PERFORM GENERATED WORKOUT COMPONENT, DISPLAYS AN OVERVIEW OF THE EXERCISES, WEIGHTS, AND REPS THAT THE USER HAS COMPLETED IN THE WORKOUT PERFORMED.
...
const CompletedGeneratedWOverview = ({route}, props) => {
  ...
  // THE ARRAY CONTAINING THE EXERCISES AND DATA OF THE GENERATED WORKOUT THAT THE USER HAS COMPLETED PASSED AS A PROP TO THIS SCREEN:
  const generatedWorkoutName = route.params.generatedWorkoutName;
  const generatedWorkoutArray = route.params.generatedWorkoutArray;
  // USESTATE TO DISPLAY THE DATA OF THE WORKOUT THE USER PERFORMED IN A FLATLIST
  let [flatListItems, setFlatListItems] = useState([]);

  //USESTATES FOR THE DATA OF THE WORKOUT THE USER HAS PERFORMED
  ...
  // FUNCTION FOR SUMMING UP THE TARGET REPS IN THE GENERATED WORKOUT ARRAY THAT THE USER PERFORMED
  ...
  function calculateRepsDifference(generatedWorkoutArray, reps) {
    const totalTargetReps = sumRepsInWorkoutArray(generatedWorkoutArray);
    const totalActualReps = formatColumnDataThenSum(reps);
    const differenceReps = totalActualReps - totalTargetReps;
    return differenceReps;
  }

  // FUNCTION FOR SUMMING UP THE TARGET SETS IN THE GENERATED WORKOUT ARRAY THAT THE USER PERFORMED
  ...
  function calculateSetsDifference(generatedWorkoutArray, flatListItems) {
    const totalTargetSets = sumSetsInWorkoutArray(generatedWorkoutArray);
  ...
  function calculateTargetSetsXReps(generatedWorkoutArray) {
    const totalTargetSets = sumSetsInWorkoutArray(generatedWorkoutArray);
    const totalTargetReps = sumRepsInWorkoutArray(generatedWorkoutArray);
  ...
  function calculateActualSetsXReps(flatListItems, reps) {
    const actualSets = flatListItems.length;
    const actualReps = formatColumnDataThenSum(reps); // YOU DON'T SUM THE REPS, YOU JUST GET THE AVERAGE NUMBER OF REPS DONE ACROSS ALL SETS AND THEN
      ...
      {loading ? (
        ...
        onPress={() => {
          InsertData(
            tableNameDate,
            String(
              calculateSetsDifference(generatedWorkoutArray, flatListItems),
            ),
            String(calculateRepsDifference(generatedWorkoutArray, reps)),
            ...
            String(calculateTargetSetsXReps(generatedWorkoutArray)),
          );
          navigation.replace('Fitness Home');
        }}>
        Submit Workout: ({generatedWorkoutName})
      </Button>
...
export default CompletedGeneratedWOverview;
