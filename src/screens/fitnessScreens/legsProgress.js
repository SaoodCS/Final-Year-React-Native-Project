...
// COMPONENT: DISPLAYS BOTH THE DEFAULT LEGS EXERCISES AND USER-CREATED LEGS EXERCISES IN ONE FLATLIST FOR THE USER TO PRESS ON AND VIEW THEIR PROGRESS ON THE DISPLAYEXPROGRESS COMPONENT

// DB TO GET AND DISPLAY USER CREATED EXERCISES
var db = openDatabase({name: 'userExercises.db'});

const LegsProgress = ({route}, props) => {
  ...
  useEffect(() => {
    db.transaction(tx => {
      tx.executeSql(
        'SELECT * FROM user_exercises WHERE exercise_musclegroup = "Legs"',
        [],
        (tx, results) => {
          var temp = [];
  ...
  function filterOutRows(database) {
    ...
    const result6 = result5.filter(item => item.MuscleGroup != 'Shoulders');
    const result7 = result6.filter(item => item.MuscleGroup != 'Full Body');

    return result7;
  }
  const legExercises = filterOutRows(exercisesdb);
  ...
  const concatDefaultNSavedExercises = legExercises.concat(changeLabels);

  return (
    <View style={{backgroundColor: '#171717', height: 700}}>
      <ScrollView>
        <CustomHeaderWithBack
          pageName="Legs Exercises Progress"
                  ...
                  <Button
                    ...
                    onPress={() => {
                      navigation.navigate('Display Progress', {
                        nameOfPreviousScrn: 'Legs Progress',
