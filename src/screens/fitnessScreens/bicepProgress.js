...
// COMPONENT: DISPLAYS BOTH THE DEFAULT BICEP EXERCISES AND USER-CREATED BICEP EXERCISES IN ONE FLATLIST FOR THE USER TO PRESS ON AND VIEW THEIR PROGRESS ON THE DISPLAYEXPROGRESS COMPONENT

// DB TO GET AND DISPLAY USER CREATED EXERCISES
var db = openDatabase({name: 'userExercises.db'});

const BicepProgress = ({route}, props) => {
  ...
  useEffect(() => {
    db.transaction(tx => {
      tx.executeSql(
        'SELECT * FROM user_exercises WHERE exercise_musclegroup = "Biceps"',
        [],
        (tx, results) => {
          var temp = [];
  ...
  function filterOutRows(database) {
    ...
    const result2 = result.filter(item => item.MuscleGroup != 'Shoulders');
  ...
  const bicepExercises = filterOutRows(exercisesdb);
  ...
  const concatDefaultNSavedExercises = bicepExercises.concat(changeLabels);

  return (
    <View style={{backgroundColor: '#171717', height: 700}}>
      <ScrollView>
        <CustomHeaderWithBack
          pageName="Bicep Exercises Progress"
                  ...
                  <Button
                    ...
                    onPress={() => {
                      navigation.navigate('Display Progress', {
                        nameOfPreviousScrn: 'Bicep Progress',
