...
const AbsProgress = ({route}, props) => {
  ...
  useEffect(() => {
    db.transaction(tx => {
      tx.executeSql(
        'SELECT * FROM user_exercises WHERE exercise_musclegroup = "Abs"',
        [],
        (tx, results) => {
          var temp = [];
  ...
  function filterOutRows(database) {
    ...
    const result4 = result3.filter(item => item.MuscleGroup != 'Shoulders');
  ...
  const absExercises = filterOutRows(exercisesdb);
  ...
  const concatDefaultNSavedExercises = absExercises.concat(changeLabels);

  return (
    <View style={{backgroundColor: '#171717', height: 700}}>
      <ScrollView>
        <CustomHeaderWithBack
          pageName="Abs Exercises Progress"
                  ...
                  <Button
                    ...
                    onPress={() => {
                      navigation.navigate('Display Progress', {
                        nameOfPreviousScrn: 'Abs Progress',
