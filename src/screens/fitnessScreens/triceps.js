...
// COMPONENT: DISPLAYS BOTH THE DEFAULT TRICEPS EXERCISES AND USER-CREATED TRICEPS EXERCISES IN ONE FLATLIST FOR THE USER TO ADD TO THE WORKOUT THEY ARE CREATING:
...
var db2 = openDatabase({name: 'userWorkouts.db'}); //DATABASE FOR THE USER ADDING EXERCISES TO THEIR WORKOUT REGIMEN

const Triceps = ({route}, props) => {
  ...
  //USESTATE FOR THE USERS SAVED EXERCISES TABLE IN THE SQLITE DATABASE:
  ...
  useEffect(() => {
    db.transaction(tx => {
      tx.executeSql(
        'SELECT * FROM user_exercises WHERE exercise_musclegroup = "Triceps"',
        (tx, results) => {
          var temp = [];
  ...
  const concatDefaultNSavedExercises = tricepsExercises.concat(changeLabels);
        ...
        <View
          ...
          <View
            ...
            <Text style={{fontSize: 20, color: 'black'}}>
              Add Tricep Exercises For: {actualWorkoutName}
            </Text>
          </View>
        </View>
